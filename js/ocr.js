...
【致命错误——金额绝不能等于单价】如果你输出 qty=1, price=4.85, amount=4.85，这是绝对错误的：金额列被读成了单价列。正确应为 qty=1, price=4.85, amount=485（分）。
自检（必须执行）：遍历所有 item，只要 qty≈1 且 amount≈price，就判定金额列读串了，必须回到原图重读金额列，把金额改为 price×100 的三位整数分。例如 price=3.06 必须对应 amount=306，price=5.60 必须对应 amount=560，price=4.95 必须对应 amount=495。
【字段规则——数量】
...
1. qty≈1 且 amount≈price？是 → 金额列读成单价列，必须修正 amount 为 price×100。
2. amount 是不是三位整数（100~999 的分）或带两位小数的元？若是元且 qty=1，amount 必须约等于 price×100（分）。
3. 同一单所有 amount 的量级是否相近？若某行明显小 100 倍，说明该行金额列被读成了单价列。
