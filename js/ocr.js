...
(function (g) {
  ...
  function compress(file, maxW, opts) {
    ...
    var defW = isMobile ? 1600 : 1800; // 电脑端默认 1800，原图识别足够
...
【数学校验——金额优先】
1. 金额（amount）以单据上手写/印刷数字为准，最高优先级。
2. 数量 × 单价 必须约等于金额，允许 ±0.02 的四舍五入误差。
3. 当单价经数据库修正后，数量应由「金额 ÷ 单价」反推（手写数量 1/2 最常见）。
...
【手写数字陷阱】序号列不是金额；同一单金额量级应相近，某行明显小一个量级很可能是小数点读错；不要把金额列数字误填到数量列。
