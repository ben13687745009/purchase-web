...
(function (g) {
  ...
  function compress(file, maxW, opts) {
    ...
    return new Promise(function (res, rej) {
          ...
          if (sharp >= 12) {
            ...
            try {
              ...
              for (var p = 0; p < len; p += 4) {
                ...
                // 仅在较深的笔迹（gray<150）做轻微加粗，避免手写蓝字相邻数字粘连吞掉小数点/串行
                if (gray < 150) {
                  cd[p]   = Math.max(0, Math.min(255, cd[p]   - dr * 0.18));
                  cd[p+1] = Math.max(0, Math.min(255, cd[p+1] - dg * 0.18));
                  cd[p+2] = Math.max(0, Math.min(255, cd[p+2] - db * 0.18));
                }
              }
      ...
      var render = (isMobile || opts.enhance) ? enhancedRender : simpleRender;
...
【输出格式】严格只输出一个 JSON 对象，不要任何解释、前言、备注、markdown 代码块之外的内容：
{
  ...
  "items": [{"name":"商品名称（原样输出名称列文字，包括尾字的包/盒/箱等；不要把规格列混进来）","unit":"单位，如箱/包/斤/kg/瓶/袋/盒/罐/支/件/个","qty":数量,"price":单价,"amount":金额,"blur":["模糊字段名"],"conf":0.0~1.0}]
}
...
【表格列顺序——必须严格遵守】单据表格的列顺序固定为：商品名称 → 商品规格（忽略，不输出） → 单位 → 数量 → 单价 → 金额。你必须按「商品名称 / 单位 / 数量 / 单价 / 金额」这五列一一对应输出 qty/price/amount，严禁读取或输出「商品规格」列的内容，严禁把规格列里的数字（如 890G*48、120g*100）填到数量或单价里，严禁把「金额列」的数字填到 price 字段，严禁把「单价列」的数字填到 qty 字段，严禁把「数量列」的数字填到 amount 字段。列对调是最常见错误，提取每行时请用手指从左到右对准列名再读数。
【分类清单】${catsStr}
【商品库参考（字迹完全模糊或字段缺失时辅助补全；有同款商品时直接采用库内标准单价）】${ctxStr}
【核心规则——原样读取，不区分元和分】
1. 本单据的单价列和金额列使用统一单位，表格里写什么数字就输出什么数字，不要做任何「分→元」或「元→分」的换算，不要给小数点，不要把 560 读成 5.60，也不要把 485 读成 4.85。
2. 金额列（amount）必须原样读取单据金额格里的数字，最高优先级，严禁用「数量×单价」去计算或替换金额列。例如原图金额列写的是 560，就必须输出 amount=560；写的是 612 就必须输出 amount=612。
3. 单价列（price）同样必须原样读取单据单价格里的数字。例如原图单价列写的是 560，就必须输出 price=560；写的是 306 就必须输出 price=306。绝对禁止因为单价≥100 就自动除以 100。
4. 数量 × 单价 必须约等于 金额（如 1×560=560、2×306=612）。如果算出来不相等，说明列读串了（比如把金额读到了单价列），必须重新对齐本行。注意：校验仅用于发现错位，绝不用「金额÷单价」去改写已经识别到的数量。
5. 商品名称列里的文字必须原样输出，包括名称末尾的「包/盒/箱/袋/罐/瓶/支/件/个」等字（例如「方包」「牛角包」「菠萝包」「长方盒」必须完整输出为 name，不能把「包」「盒」删掉或移到 unit 字段）。只有明确属于「单位」列的内容才填 unit；不要把「商品规格」列里的数字/重量/包装信息（如 890G*48罐/箱、120g*100包、1KG、1L*12盒/箱）塞进 name。仅字迹完全模糊或名称缺失时，才用商品库比对补全名称；字段缺失时可用商品库历史单价/名称推算，但严禁编造原图中不存在的商品。
6. 禁止输出单据不存在的商品；同一张单据内部出现完全重复条目时只保留一条；禁止跨单据合并汇总。
7. 不要把单据底部「合计/总计/运费/小计」行当成普通商品行输出。
8. 所有数字最多保留 2 位小数；本单几乎没有小数，看到整数直接输出整数。
9. 绝对禁止行与行之间的数字串读：第 2 行的金额/单价/数量必须是第 2 行的，不能把第 1 行的单价 485 误读到第 2 行金额位置，也不能把第 3 行的数量读到第 2 行。每提取一行后，用手指从左到右对准「名称→单位→数量→单价→金额」五列，确认五个值在同一水平线上。
10. 提取完所有行后，必须做三值一致性自检：对每一行计算「数量 × 单价」，结果必须约等于「金额」。若不相等，说明列读串了，必须重新对齐本行后再次输出。
11. 严禁为了让 amount=qty×price 成立，把数量和单价互相缩放100倍。例如原图「数量1、单价855、金额855」的商品，绝对禁止输出 qty=100、price=8.55、amount=855；也不能输出 qty=1、price=8.55、amount=8.55。商品规格列（如 890G*48罐/箱、120g*100包、1KG、1L*12盒/箱）完全忽略，不要读取，更不要把它里面的数字填到数量列或单价列；数量列只看单独的数量格。
12. 如果你识别到单价是 8.55 而金额是 855、数量是 100，说明你把原图单价 855 误读成了 8.55，同时把规格列或名称尾部的数字误当成数量。正确做法：price 还原为 855，qty 按金额÷单价=1 输出；如果看不清，宁可输出 null 也不要硬凑。
【字段规则——单价】
单价列（price）必须原样读取单据单价格里的数字，本单常见为 100~999 的整数（如 560/485/306/516）。不要因为它看起来大就除以 100，也不要臆造小数点。只有原图确实看清是小数点才输出小数；看不清时输出 null 或按商品库参考价推算，严禁把整数当小数。
【致命错误——金额绝不能等于单价，也绝不能等于数量×单价】
如果你输出 qty=1, price=560, amount=560，但原图金额列明明写的是 560 而单价列写的是 560，这是允许的，因为本单统一单位。
如果你输出 qty=2, price=306, amount=612，但原图金额列写的是 612，这是正确的。
如果你输出 qty=2, price=306, amount=6.12，这是绝对错误的：金额列被你用「数量×单价」自算了，没读原金额列。正确应为 qty=2, price=306, amount=612。
自检（必须执行）：遍历所有 item，只要 amount 明显小于 qty×price（如小 100 倍）或 amount 是两位小数而 qty×price 是整数，就判定金额列没读原图或读串列，必须回到原图重读金额列原数字。
【字段规则——数量】
数量列（qty）在餐饮送货单里几乎总是整数 1、2、3…，偶尔有 1.5、2.5、144.5 等一位小数（称重商品常见）。严禁把单价列/金额列的数字填到数量列。只要原图数量格看清了，就原样输出，不要为了让 amount=qty×price 完全相等而硬改成除法余数（如 144.5454）。如果数量列看不清，宁可输出 null 也不要猜一个跟单价相近的小数。
【数学校验——金额优先】
1. 金额（amount）以单据上手写/印刷数字为准，最高优先级；不得以任何理由用 qty×price 去覆盖或修改金额列。
2. 数量 × 单价 必须约等于金额，允许 ±0.02 或 ±1% 的四舍五入误差；这一公式只用于「校验」和「补全缺失字段」，不能用于改写已经看清的金额列、数量列或单价列。
3. 当单价、金额都清晰时，数量应由「金额 ÷ 单价」反推校验（手写数量 1/2 最常见）；但如果原图数量列也清晰，则以识别到的数量为准，不要硬改成除法余数（如 144.5454）。
4. 如果原单金额列缺失或完全模糊，才用「数量×单价」推算金额，并在 blur 标注「金额推算」。
【数据库比对逻辑】
以商品库为辅助参考，但不得覆盖原图清晰可辨的数字：
1. 品名近似匹配成功后，如果图片单价列字迹模糊或缺失，可用商品库标准单价补全；如果图片单价列清晰可辨，必须优先采用图片原数字。
2. 严禁因为商品库单价是 4.85 就把图片清晰识别的 485 改成 4.85。商品库只是参考，原图数字才是权威。
3. 匹配商品库时只使用商品名称列文字，不要把商品规格列内容也拿去匹配；规格列与品名判定无关。
4. 图片识别单价 ≥ 100 时，不要自动除以 100；只有当字迹确实模糊、且商品库同款单价明显小 100 倍时，才作为候选参考。
【手写打印混合单规则】
...
C. 金额列如果模糊、被涂改或只写了一半（如冬菇金额仅写一个「4」），不能留空或输出 0；应结合数量列、商品库历史单价，按 数量×单价 推算金额。
【数字校验优先级】
① 金额列清晰时优先采信金额。
② 品名匹配到商品库时，单价列模糊/缺失才用数据库标准单价补全；单价列清晰时优先采用原图数字。
③ 金额、单价、数量都可读时，三者相互校验，但始终以识别到的原值为准。数量应为整数或一位小数（如 1、2、1.5、144.5、289.2、62.7）。校验差值在 0.02~1.0 之间视为正常手写/称重误差；绝不要把识别数量硬改成「金额÷单价」的除法余数（如 62.6963、289.2453）。
...
【手写数字陷阱】序号列不是金额；同一单金额量级应相近，某行明显小一个量级很可能是小数点读错；不要把金额列数字误填到数量列；不要把单价列数字误填到数量列；金额列出现明显小于 qty×price 的数字时，100% 是列错位，必须回到原图重新读取金额列原数字。
【输出前最终自检——必须执行】对输出的每个 item 依次检查，任何一项触发都必须返回原图重读并修正：
1. amount 是否约等于 qty×price？不相等 → 列读串了，必须重新对齐。
2. price 是否被读成了两位小数（如 5.60/4.85/3.06）而原图单价格明明是整数 560/485/306？是 → 单价列被错误除以 100，必须修正回原整数。
3. amount 是否被读成了两位小数（如 5.60/4.85/3.06）而原图金额格明明是整数 560/485/306？是 → 金额列被错误除以 100，必须修正回原整数。
4. qty 是否 ≥100 且 price 是两位小数（如 qty=100、price=8.55、amount=855）？是 → 数量和单价被互相缩放100倍，必须修正为 qty=1、price=855、amount=855。
5. items 数量是否等于原图行数？少一行就是漏行，必须补齐。
...
【正确识别示例——照此格式输出，一行不许漏】
下面是一张汁水送货单，表格列顺序固定为：名称→单位→数量→单价→金额。本单单价和金额统一单位，都是整数。共 4 行商品：
  咖喱    1   560   560
  瑞士汁  1   485   485
  黑椒    2   306   612
  冬阴功  1   516   516
正确 JSON（4 行必须 4 个 item，单价和金额都原样输出整数）：
{
  "date": "7.21",
  "category": "汁水",
  "items": [
    {"name":"咖喱","unit":"箱","qty":1,"price":560,"amount":560},
    {"name":"瑞士汁","unit":"箱","qty":1,"price":485,"amount":485},
    {"name":"黑椒","unit":"箱","qty":2,"price":306,"amount":612},
    {"name":"冬阴功","unit":"箱","qty":1,"price":516,"amount":516}
  ]
}
★ 单价列 560/485/306/516 都是原图整数，直接输出，不要÷100、不要加小数点。
★ 金额列 560/485/612/516 同样原样输出整数，不要÷100、不要加小数点。
★ 三值校验：1×560=560、1×485=485、2×306=612、1×516=516，必须全部成立。
★ 4 行商品，items 就必须是 4 个；少一个就是漏行错误，必须回到原图补齐。
【正确识别示例2——5 行也不能漏】
另一张单据 5 行：番茄肉酱/肉酱/黑椒汁/烧汁/茄汁，单价分别 414/495/306/294/390，金额分别 414/495/306/294/390。也必须恰好输出 5 个 item，顺序从上到下，禁止并两行、跳行。
【正确识别示例3——面包/包装品类名称必须保留尾字】
下面是一张面包送货单，商品名称列文字必须原样输出：
  方包      2   17   34
  牛角包   10    5   50
  菠萝包    5    3   15
  餐包      1    8    8
...
{
  "items": [
    {"name":"方包","unit":"个","qty":2,"price":17,"amount":34},
    {"name":"牛角包","unit":"个","qty":10,"price":5,"amount":50},
    {"name":"菠萝包","unit":"个","qty":5,"price":3,"amount":15},
    {"name":"餐包","unit":"个","qty":1,"price":8,"amount":8}
  ]
}
★ 名称里的「包」字是商品名称的一部分，必须保留在 name 中，不能拆成 name="方" unit="包"。
★ 包装品如「长方盒」「正方盒」「打包盒」同理，「盒」字必须保留在 name 中。
【常见汁水品名】单据中的「瑞士汁」「咖喱汁」「黑椒汁」「茄汁」「烧汁」「冬阴功」「肉酱」等名称必须逐字辨认，不要把「瑞士汁」误读成「咖士汁」，不要把「黑椒」漏掉「汁」字，不要把「冬阴功」误读成其他酱名。
【单价大数处理】本单单价和金额统一单位，原图单价格写 560/485/306/516 就原样输出 560/485/306/516，不要因为 ≥100 就除以 100。商品库只是参考，原图清晰数字优先。
