...
4. 数量 × 单价 必须约等于 金额（如 1×560=560、2×306=612）。如果算出来不相等，说明列读串了（比如把金额读到了单价列），必须重新对齐本行。
...
③ 金额、单价、数量都可读时，三者相互校验，数量应为整数或一位小数（如 1、2、1.5、144.5）。若数量算出来是 100、410、57.14 等明显除法余数，而原图数量格明明是一位小数/整数，则以原图识别到的数量为准，仅标核对，不要硬改成除法余数。
