...
5. 名称规格严格忠于原图文字，单位单独填 unit，不要把「箱/包/斤/kg/瓶/袋/盒/罐/支/件/个」等塞进 name。仅字迹完全模糊时才用商品库比对补全名称；字段缺失时可用商品库历史单价/名称推算，但严禁编造原图中不存在的商品。
...
11. 严禁为了让 amount=qty×price 成立，把数量和单价互相缩放100倍。例如原图「数量1、单价855、金额855」的商品，绝对禁止输出 qty=100、price=8.55、amount=855；也不能输出 qty=1、price=8.55、amount=8.55。名称规格里的数字（如 120g*100、500g*20）只是包装规格，不要把它填到数量列；数量列只看单独的数量格。
12. 如果你识别到单价是 8.55 而金额是 855、数量是 100，说明你把原图单价 855 误读成了 8.55，同时把规格里的 100 误当成数量。正确做法：price 还原为 855，qty 按金额÷单价=1 输出；如果看不清，宁可输出 null 也不要硬凑。
【字段规则——单价】
...
3. 图片识别单价 ≥ 100 时，不要自动除以 100；只有当字迹确实模糊、且商品库同款单价明显小 100 倍时，才作为候选参考。
【手写打印混合单规则】
