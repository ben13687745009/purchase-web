...
(function (g) {
  ...
  function buildPrompt(ctx, cats, hintDate, hintCat) {
    ...
    return `你是餐饮采购单表格结构化提取助手。请识别图片中的手写采购单/送货单。
【读取范围】只读取红色划线/方框内的商品表格，以及表头上方「录单日期/送货日期」。绝对不要读取或输出：制单人/经办人/送货人/收货单位/仓库、送货日期页脚/打印时间/第N页、总重量kg/总件数、单据编号/电话/二维码/地址。
【输出格式】严格只输出一个 JSON 对象，不要任何解释、前言、备注：
{
  "date": "录单日期【必填】，格式 m.d，如 6.1、3.12，从表头上方提取",
  "category": "分类名，从下方分类清单选最匹配的一个；判断不了填空字符串",
  "items": [{"name":"名称和规格","qty":数量,"price":单价,"amount":金额,"blur":["模糊字段"],"conf":0.0~1.0}]
}
【分类清单】${catsStr}
【商品库参考（仅字迹完全模糊时比对）】${ctxStr}
【核心规则——忠于原图】
1. 名称规格严格忠于原图；仅字迹完全模糊才用商品库比对；不确定字段输出 null，严禁编造。
2. 数量=1 可省略；金额空白填 0。
3. 所有数字最多保留 2 位小数。
【数字校验优先级】
A. 原图清晰以原图为准，不许篡改。
B. 金额清晰优先采信金额；库历史单价仅作辅助校验，不覆盖原图单价。
C. 仅数量或单价模糊才允许 金额÷单价 反推；原图数量可读绝不改数量/单价。
D. 自检：qty×price≈amount（偏差<10%），偏差>50% 重读原图。
【防漏行】先数行数 N（看序号列最后一个数），items 必须恰好 N 个，逐行提取不跳行、不并两行、不把表头当数据。
【手写数字陷阱】小数点不凭空插入；序号列非金额；同一单金额量级应相近。${hintDateStr}${hintCatStr}
只读红框内商品表格，金额最重要不能读错。`;
  }
...
【商品库参考（仅字迹完全模糊时比对）】${ctxStr}
【核心规则——严格忠于原图】
1. 名称规格严格忠于原图文字。只有字迹完全模糊、无法辨认时，才可调用商品库比对补全；模糊但能猜出，填最好猜测并在 blur 标记、conf 填低值（0.2~0.5）。
2. 识别不确定的字段输出 null，严禁编造数据。
3. 数量=1 时可省略（默认 1），但金额列必填；金额空白填 0。
4. 所有数字最多保留 2 位小数。
【数字校验优先级】
A. 原图清晰：必须以原图为准，不许篡改。
B. 金额清晰：优先采信金额。商品库历史单价仅作异常辅助校验，不能覆盖原图单价。
C. 只有数量或单价字迹模糊时，才允许 金额÷单价 反推缺失项；原图数量可读，绝不允许为凑等式修改数量或单价。
D. 输出前自检：每行 qty×price 应 ≈ amount（偏差<10%）；偏差>50% 说明小数点或金额读错，重新读原图。
【防漏行（两步法）】
...
【手写汉字品名】逐笔画辨认，不要只看大概；与商品库相似度极高的优先用标准名；常见误读（实↔深、冬↔冻、耳↔鱼、边↔过）请额外小心；包装单位（整箱/500g）读进 name。${hintDateStr}${hintCatStr}
再次强调：只读红框内商品表格，金额最重要不能读错，小数点更不能凭空插入。`;
  }
