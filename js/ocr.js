...
【输出格式】严格只输出一个 JSON 对象，不要任何解释、前言、备注、markdown 代码块之外的内容：
{
  ...
  "items": [{"name":"商品名称（不要含单位，不要含规格）","unit":"单位，如箱/包/斤/kg/瓶/袋/盒/罐/支/件/个","qty":数量,"price":单价,"amount":金额,"blur":["模糊字段名"],"conf":0.0~1.0}]
}
...
5. 商品名称严格只读取「商品名称」列，不要读取或混入「商品规格」列的内容，不要把规格里的数字/重量/包装信息（如 890G*48罐/箱、120g*100包、1KG、1L*12盒/箱）塞进 name。单位单独填 unit，不要把「箱/包/斤/kg/瓶/袋/盒/罐/支/件/个」等塞进 name。仅字迹完全模糊时才用商品库比对补全名称；字段缺失时可用商品库历史单价/名称推算，但严禁编造原图中不存在的商品。
...
2. 数量 × 单价 必须约等于金额，允许 ±0.02 或 ±1% 的四舍五入误差；这一公式只用于「校验」和「补全缺失字段」，不能用于改写已经看清的金额列。
3. 当单价、金额都清晰时，数量应由「金额 ÷ 单价」反推校验（手写数量 1/2 最常见）。
...
③ 金额、单价、数量都可读时，三者相互校验，数量应为整数或简单小数（如 1、2、1.5）。若数量算出来是 100、410、57.14 等明显除法余数，说明单价或数量识别错位，应以金额和清晰单价为准重新修正。
