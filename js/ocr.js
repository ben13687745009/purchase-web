...
4. 所有数字最多保留 2 位小数。
5. 绝对禁止行与行之间的数字串读：第 2 行的金额/单价/数量必须是第 2 行的，不能把第 1 行的单价 485 误读到第 2 行金额位置，也不能把第 3 行的数量读到第 2 行。每提取一行后，用手指从左到右对准「名称→单位→数量→单价→金额」五列，确认五个值在同一水平线上。
6. 提取完所有行后，必须做三值一致性自检：对每一行计算「数量 × 单价」，结果必须约等于「金额」（误差 ≤0.02 或 ≤1%）。若不相等，说明列读串了，必须重新对齐本行后再次输出。
【字段规则——金额列】
...
自检（必须执行）：遍历所有 item，只要 amount≈qty×price（即金额看起来刚好是数量乘单价的小数结果），就判定金额列没读原图，必须回到原图重读金额列的整数分。金额列必须输出原图上的整数（如 306/485/612/516/414/495/560），绝不能输出 3.06/4.85/6.12/5.16 这类自算小数。
【字段规则——数量】
...
【输出前最终自检——必须执行】对输出的每个 item 依次检查：
1. amount 是否约等于 qty×price？是 → 金额列被你用数量×单价自算了，必须回到原图重读金额列的整数分（如 612/516/414），禁止输出 6.12/5.16/4.14 这种自算小数。
2. qty≈1 且 amount≈price？是 → 金额列读成单价列，必须修正 amount 为 price×100。
3. amount 是不是三位整数（100~999 的分）或带两位小数的元？若是元且 qty=1，amount 必须约等于 price×100（分）。
4. 同一单所有 amount 的量级是否相近？若某行明显小 100 倍，说明该行金额列被读成了单价列。
