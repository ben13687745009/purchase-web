...
【核心规则——金额列最高优先级】
1. 金额列（amount）必须原样读取单据金额格里的数字，最高优先级，严禁用「数量×单价」去计算或替换金额列。例如原图金额列写的是 560，就必须输出 amount=560；写的是 485 就必须输出 amount=485；绝对禁止输出 amount=5.60 或 4.85 这种带两位小数的元。
2. 本单据金额列全部为整数「分」（常见 100~999 的三位整数，如 560/485/612/516/414/495/306）。金额列没有小数点，不要给金额列加小数点，不要把 560 读成 5.60，不要把 485 读成 4.85。如果你输出 amount=5.60/4.85/3.06/5.16/4.95/2.94 等两位小数，说明你把金额列读成了单价列，这是绝对错误，必须回到原图重读金额列的三位整数。
3. 金额列只在极少数情况下是带两位小数的元（如 17.13/24.50）， ONLY 当原图金额格明确印有小数点和小数位时才输出小数；否则一律按整数分原样输出。
4. 金额列禁止等于单价列：如果某行 price=4.85 且 amount=4.85，说明金额列被读成单价列，必须修正 amount=485（分）。
5. 金额列禁止等于数量×单价：如果 qty=2, price=3.06, amount=6.12，说明金额列被你用公式自算了，必须修正 amount=612（分）。
6. 名称规格严格忠于原图文字，单位单独填 unit，不要把「箱/包/斤/kg/瓶/袋/盒/罐/支/件/个」等塞进 name。仅字迹完全模糊时才用商品库比对补全名称；字段缺失时可用商品库历史单价/名称推算，但严禁编造原图中不存在的商品。
7. 禁止输出单据不存在的商品；同一张单据内部出现完全重复条目时只保留一条；禁止跨单据合并汇总。
8. 不要把单据底部「合计/总计/运费/小计」行当成普通商品行输出。
9. 所有数字最多保留 2 位小数。
10. 绝对禁止行与行之间的数字串读：第 2 行的金额/单价/数量必须是第 2 行的，不能把第 1 行的单价 485 误读到第 2 行金额位置，也不能把第 3 行的数量读到第 2 行。每提取一行后，用手指从左到右对准「名称→单位→数量→单价→金额」五列，确认五个值在同一水平线上。
11. 提取完所有行后，必须做三值一致性自检：对每一行计算「数量 × 单价」，结果必须约等于「金额÷100」（因为金额是分，1 元=100 分）。若不相等，说明列读串了，必须重新对齐本行后再次输出。
【字段规则——单价】
...
如果你输出 qty=1, price=4.85, amount=4.85，这是绝对错误的：金额列被读成了单价列。正确应为 qty=1, price=4.85, amount=485（分）。
如果你输出 qty=2, price=3.06, amount=6.12，这也是绝对错误的：金额列被你用「数量×单价」自算了，没读原金额列。正确应为 qty=2, price=3.06, amount=612（分）。
自检（必须执行）：遍历所有 item，只要 amount≈qty×price 或 amount≈price，就判定金额列没读原图或读串列，必须回到原图重读金额列的整数分。金额列必须输出原图上的整数（如 306/485/612/516/414/495/560），绝不能输出 3.06/4.85/6.12/5.16 这类两位小数。
【字段规则——数量】
...
1. amount 字段是否带两位小数（如 5.60/4.85/3.06）？是 → 金额列被读成单价列，必须修正为整数分（560/485/306）。
2. amount 是否约等于 qty×price？是 → 金额列被你用数量×单价自算了，必须回到原图重读金额列的整数分（如 612/516/414）。
3. qty≈1 且 amount≈price？是 → 金额列读成单价列，必须修正 amount 为 price×100（分）。
4. amount 是不是 100~999 的整数分？若不是，且原图金额列清晰无小数点，说明读串/读错，必须重读。
5. 同一单所有 amount 的量级是否相近？若某行明显小 100 倍，说明该行金额列被读成了单价列。
6. items 数量是否等于原图行数？少一行就是漏行，必须补齐。
...
【正确识别示例——照此格式输出，一行不许漏】
下面是一张汁水送货单，表格列顺序固定为：名称→单位→数量→单价(元,带两位小数)→金额(分,三位整数)。共 4 行商品：
  咖喱    1   5.60   560
  瑞士汁  1   4.85   485
  黑椒    2   3.06   612
  冬阴功  1   5.16   516
正确 JSON（4 行必须 4 个 item，金额原样为分整数，单价带两位小数）：
{
  "date": "7.21",
  "category": "汁水",
  "items": [
    {"name":"咖喱","unit":"","qty":1,"price":5.60,"amount":560},
    {"name":"瑞士汁","unit":"","qty":1,"price":4.85,"amount":485},
    {"name":"黑椒","unit":"","qty":2,"price":3.06,"amount":612},
    {"name":"冬阴功","unit":"","qty":1,"price":5.16,"amount":516}
  ]
}
★ 金额列 560/485/612/516 都是「分」，原样输出三位整数，不要加小数点、不要÷100。表格直接显示该原值，后端不再换算。
★ 单价列 5.60/4.85/3.06/5.16 都带两位小数，请逐位辨认小数点位置。
★ 4 行商品，items 就必须是 4 个；少一个就是漏行错误，必须回到原图补齐。
【正确识别示例2——5 行也不能漏】
另一张单据 5 行：番茄肉酱/肉酱/黑椒汁/烧汁/茄汁，单价分别 4.14/4.95/3.06/2.94/3.90，金额分别 414/495/306/294/390（都是分）。也必须恰好输出 5 个 item，顺序从上到下，禁止并两行、跳行。
